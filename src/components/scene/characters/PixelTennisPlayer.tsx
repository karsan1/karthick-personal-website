...
import { COURT_DIMENSIONS, sampleReducedMotionCharacterPose, sampleReducedMotionCharacterRootTransform, type CharacterNarrativeSample, type CharacterRootTransformSample, type CharacterSide, type NarrativeProgress } from "@/animations/prototypeMotion";
import { sampleContinuousRallyCharacter, sampleContinuousRallyRootTransform } from "@/animations/continuousRally";
...
/** Maps the master scroll progress to discrete frames from the continuous rally. */
export function PixelTennisPlayer({ progress, side, reducedMotion }: PixelTennisPlayerProps) {
  ...
  const transform = useRef<CharacterRootTransformSample>({ positionX: side === "a" ? -0.12 : 0.14, positionY: 0, positionZ: side === "a" ? -COURT_DIMENSIONS.playerBaselineZ : COURT_DIMENSIONS.playerBaselineZ, rotationY: 0, rotationZ: 0, scaleY: 1 });
  const previousCell = useRef({ column: -1, row: -1 });
  useFrame(() => {
    const group = root.current;
    if (!group) return;
    const rootSample = reducedMotion ? sampleReducedMotionCharacterRootTransform(side, transform.current) : sampleContinuousRallyRootTransform(progress.current.value, side, transform.current);
    ...
    const narrative = reducedMotion ? sampleReducedMotionCharacterPose(pose.current) : sampleContinuousRallyCharacter(progress.current.value, side, pose.current);
