...
// Outside the doubles alley and in front of the stadium side-wall silhouette.
const SIDELINE_OFFICIALS: readonly Placement[] = [
  // Kept clear of the Resume and Experience hotspot volumes, including the
  // 1.05 m billboard width and 0.30 m shadow depth.
  { position: [-4.52, 0.02, -1.2], variant: 0 }, { position: [4.52, 0.02, -1.2], variant: 1, scale: 0.98 },
  { position: [-4.52, 0.02, 3.65], variant: 3, scale: 0.98 }, { position: [4.52, 0.02, 3.65], variant: 2 },
] as const;
...
function sideCount(qualityTier: QualityTier) {
  if (qualityTier === "high") return 4;
  if (qualityTier === "medium") return 2;
  return 0;
}
...
function SidelineCrew({ qualityTier }: { qualityTier: QualityTier }) {
  return <group name="pixel-sideline-crew">{SIDELINE_OFFICIALS.slice(0, sideCount(qualityTier)).map(({ position, variant, scale = 1 }, index) => (
    <group key={`sideline-${index}`} position={[position[0], position[1], position[2]]}>
      ...
      <PixelAtlasSprite src={VENUE_PIXEL_SPRITES.sideline} columns={4} rows={1} column={variant} row={0} width={1.05 * scale} height={1.42 * scale} position={[0, 0.01, 0]} alphaTest={0.42} renderOrder={2} />
    </group>
  ))}</group>;
}

function AudienceSprites({ qualityTier }: { qualityTier: QualityTier }) {
  return <group name="pixel-venue-cast"><RearAudience qualityTier={qualityTier} /><SidelineCrew qualityTier={qualityTier} /></group>;
}
...
function PixelUmpire() {
  return <group name="pixel-chair-umpire" position={[4.85, 0.02, 0.4]}>
    ...
    <PixelAtlasSprite src={VENUE_PIXEL_SPRITES.umpire} columns={1} rows={1} width={1.55} height={2.52} position={[0, 0, 0]} alphaTest={0.42} renderOrder={3} />
  </group>;
}
