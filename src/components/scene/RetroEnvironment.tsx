...
export function RetroEnvironment({ progress, qualityTier, reducedMotion }: { progress: MutableRefObject<NarrativeProgress>; qualityTier: QualityTier; reducedMotion: boolean }) {
  ...
  return <group name="retro-environment"><RetroCourt palette={palette} /><VenueStations palette={palette} progress={progress} reducedMotion={reducedMotion} castReady={stage >= 2} />{stage >= 1 ? <StadiumShell palette={palette} detail={detail} /> : null}{stage >= 2 ? <VenueCastAudience qualityTier={qualityTier} /> : null}</group>;
}
